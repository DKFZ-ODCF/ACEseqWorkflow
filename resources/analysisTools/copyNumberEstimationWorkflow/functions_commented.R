...
doItForOneChr = function(chr) {
	...
	for (seg in start) {
		...
		#  heterozygous SNPs od germline 0.3<BAF>0.7	
		...
		# more than 30 SNPs in segment  ==> find exact peak position with derivative of BAF density function
		if (length(sel) > 30) {
			tmp = density(dataAll$betaT[sel])
			dev = derive(tmp$x,tmp$y,width=15,sigma=9)
			...
			valid_minima = minima[minima$BAF > 0.43 & minima$BAF < 0.57, , drop = FALSE]  ## add drop, 2013-9-25
			valid_maxima = maxima[maxima$BAF > 0.43 & maxima$BAF < 0.57, , drop = FALSE]  ## add drop, 2013-9-25
			
			peaks = NA
			reason = NA
			if (nrow(valid_minima) > 0) { #has the density a minimum close to 0.5?
			
				if (nrow(valid_minima) == 1) { #only one minimum between 0.43 & 0.57?
				...
				} else { #more than one minimum between 0.43 & 0.57?
					
					peaks = "crap"
					reason = "more than one maximum between 0.43 and 0.57"
				}
			} else { # no minimum close to 0.5
				# hier sollte wenn dann nur noch ein maximum im valid bereich liegen

				if (nrow(valid_maxima) == 1) { #gibt es ein maximum zwischen 0.43 und 0.57
...
evaluate_maxima = function(maxima_left, maxima_right) {
	...
	if (nrow(maxima_left) > 0 && nrow(maxima_right) > 0) { #are there maxima left and right of the minimum
		takeIt = c()
		for (i in seq_len(length(maxima_left) / 2)) {
			for (j in seq_len(length(maxima_right) / 2)) {
				#corrected equation, before always true
				if ((0.5 - maxima_left$BAF[i] - (maxima_right$BAF[j] - 0.5)) < 0.08) { #are the maxima symmetric?
		...
		if (length(takeIt) > 0) { #symmetric maxima found
			heights_ok = FALSE
			for (h in seq_len(dim(takeIt)[1])) {
				if (abs(1 - (maxima_left$density[ takeIt[h, 1] ] / maxima_right$density[ takeIt[h, 2] ])) < 0.50) {
					heights_ok = TRUE
				}
			}
			
			if (heights_ok) { #heights are more or less the same
				peaks = 2
				reason = "two symmetric maxima"
			} else { #heights differ too much
				peaks = "crap"
				reason = "symmetric maxima differ too much in height"
			}

		} else { #no symmetric maxima found
			peaks = "crap"
		...
		peaks = "crap"
		reason = "no maxima between 0.43 and 0.57 AND no symmetric maxima left and right"
	}
